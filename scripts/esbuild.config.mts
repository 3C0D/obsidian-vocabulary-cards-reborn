...
import { copyFile, copyFilesToTargetDir, isValidPath, removeMainCss } from "./utils.mts";
...
async function handleStyles() {
	...
	const sassStylesFilePath = './src/styles.scss';
	...
	const sassStylesFileExists = await isValidPath(sassStylesFilePath);

	if (!stylesFileExists && !altStylesFileExists && !sassStylesFileExists) return 1;
	if (sassStylesFileExists) return 0;
