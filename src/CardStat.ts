...
export class CardStat {
	...
	private async createAndSaveNewId(
		app: App,
		lines: string[],
		...
		await app.vault.process(file, (content) => {
			...
			newLines[sectionInfo.lineStart] =
				newLines[sectionInfo.lineStart].trim() + ` id:${id}`;
	...
	async cleanupSavedStats(): Promise<void> {
		...
		const currentDerivatives = new Set(
			this.cardList.cards.map((card) => card.derivative)
		);
