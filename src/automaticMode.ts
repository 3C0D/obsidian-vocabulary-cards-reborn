...
export function disableButtons(el: HTMLElement): void {
	const buttons = el.querySelectorAll(
		'.voca-card_button-danger, .voca-card_button-success'
...
export async function runAutoMode(
	...
	async function runCycle(): Promise<void> {
		...
		plugin.autoModeTimer = setTimeout(async () => {
			if (!plugin.autoMode) return;
			const blurredEl = el.querySelector(
				'.voca-card_explanation-blurred'
			) as HTMLElement;
			if (blurredEl) {
				blurredEl.click();
				plugin.autoModeTimer = setTimeout(
					() => runCycle(),
					plugin.settings.explainTime * 1000
				);
			} else {
				runCycle();
			}
