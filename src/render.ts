...
export function renderTableRow(tableBody: HTMLElement, word: Card): void {
	...
	derivative.createEl('span', {
		cls: 'voca-table_derivative-text',
...
function renderTranscription(derivative: HTMLElement, transcription: string): void {
	const transcriptionEl = derivative.createEl('span', {
		cls: 'voca-table_derivative-transcription'
	});
	transcriptionEl.createEl('span', {
		cls: 'voca-table_derivative-transcription-delimiter',
		text: '/'
	});
	transcriptionEl.createEl('span', {
		cls: 'voca-table_derivative-transcription-text',
		text: transcription
	});
	transcriptionEl.createEl('span', {
		cls: 'voca-table_derivative-transcription-delimiter',
...
export async function renderCard(
	...
	playButton.addEventListener(
		'click',
		async () =>
			await toggleAutoMode(plugin, cardList, cardStat, container, ctx, source)
	);
	...
	if (plugin.settings.showContextMenuButton) {
		if (plugin.settings.useDropdownMenu) {
			...
			if (cardStat && cardList) {
				dropdown.addOption(
					'mode',
					`🔀 ${plugin.mode === 'random' ? i10n.next[userLang] : i10n.random[userLang]}`
				);
				dropdown.addOption(
					'invert',
					`🔄 ${plugin.invert ? i10n.normal[userLang] : i10n.invert[userLang]}`
				);
			}
				...
				switch (value) {
					...
					case 'mode':
						if (cardStat && cardList) {
							...
							(
								el.querySelector('.mode-div') as HTMLSpanElement
							).textContent =
								plugin.mode === 'random'
									? i10n.random[userLang]
									: i10n.next[userLang];
						}
						break;
					case 'invert':
						if (cardStat && cardList) {
							...
							await plugin.saveSettings();
							(
								el.querySelector('.invert-div') as HTMLSpanElement
							).textContent = plugin.invert
								? i10n.invert[userLang]
								: i10n.normal[userLang];
...
export function renderCardStats(
	cardEl: HTMLElement,
	cardStat: CardStat,
	card: Card,
	cardList: CardList
	...
	if (cardList.length) {
		cardEl.createEl('span', {
			cls: 'voca-card_stat-total',
			title: i10n.total[userLang],
			text: `${cardList.length.toString()} ${i10n.cards[userLang]}`
		});
	}
...
export function renderCardContent(
	...
	const contentContainer = cardEl.createEl('div', {
		cls: 'voca-card_content-container'
	});
...
export function renderCardButtons(
	plugin: VocabularyView,
	cardEl: HTMLElement,
	card: Card,
	cardStat: CardStat,
	...
	const wrong = btns.createEl('button', {
		cls: 'voca-card_button-danger',
		text: i10n.repeat[userLang]
	});
	wrong.addEventListener('click', async () => {
		await confirm(plugin, cardList, cardStat, card, el, ctx, false, src);
	});

	const success = btns.createEl('button', {
		cls: 'voca-card_button-success',
		text: i10n.iKnow[userLang]
	});
...
export function reloadButton(
	...
	const reload = buttonContainer.createEl('button', {
		cls: 'reload-container_button-reload',
		title: i10n.reload[userLang],
		text: ' ↺'
	});
...
export function mode(plugin: VocabularyView, el: HTMLElement): void {
	...
	container.createEl('div', {
		cls: 'mode-div',
		text: plugin.mode === 'random' ? i10n.random[userLang] : i10n.next[userLang]
	});
}
...
export function invert(plugin: VocabularyView, el: HTMLElement): void {
	...
	container.createEl('div', {
		cls: 'invert-div',
		text: plugin.invert ? i10n.invert[userLang] : i10n.normal[userLang]
	});
}
