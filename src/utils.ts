...
function getContentAfterCodeBlock(lines: string[], codeBlockEndLine: number): string {
	...
	const nextCodeBlockIndex = contentAfter.findIndex(
		(line) =>
...
export async function replaceLanguage(
	...
	await plugin.app.vault.process(file, (content) => {
		...
		newLines[sectionInfo.lineStart] = codeBlockHeader.includes('voca-card')
			? newLines[sectionInfo.lineStart].replace('voca-card', 'voca-table')
			: newLines[sectionInfo.lineStart].replace('voca-table', 'voca-card');
...
export function getRandomCardWithWeight(cards: Card[], cardStat: CardStat): Card {
	...
	const weightedCards = cards.map((card) => {
		...
		const weight = Math.min(
			((Math.log(wrong + 1) + baseWeight) / (right + 1)) *
				(1 + Math.random() * randomFactor),
			maxWeight
		);
		return { card, weight };
	});
...
export function createEmpty(el: HTMLElement, secondChild?: HTMLElement): void {
	const emptyElement = el.createEl('div', {
		cls: 'voca-empty',
		text: i10n.empty[userLang]
	});
	if (secondChild) {
...
export async function cleanStats(): Promise<void> {
	...
	const unusedKeys = Object.keys(this.settings.stats).filter(
		(key) => !usedIds.has(key)
	);
			...
			.onClick(async () => {
				...
				if (!isVocaCard) {
					...
					el.addEventListener('contextmenu', (event) =>
						handleContextMenu(event, plugin, el, ctx, source)
					);
				}
			})
	);
		...
		menu.addItem((item) =>
			item
				...
				.onClick(async () => {
					...
					(el.querySelector('.mode-div') as HTMLSpanElement).textContent =
						plugin.mode === 'random'
		...
		menu.addItem((item) =>
			item
				...
				.onClick(async () => {
					...
					await plugin.saveSettings();
					(el.querySelector('.invert-div') as HTMLSpanElement).textContent =
						plugin.invert ? i10n.invert[userLang] : i10n.normal[userLang];
	...
	try {
		...
		try {
			...
			try {
				...
				new Notice(
					"Context menu unavailable on this system. Enable 'Use dropdown menu' in settings as an alternative."
				);
			}
		}
	}
...
export function showDropdownActions(
	plugin: VocabularyView,
	el: HTMLElement,
	ctx: MarkdownPostProcessorContext,
	source: string,
	...
	dropdown.addOption(
		'switch',
		`↔️ ${isVocaCard ? i10n.tableSwitch[userLang] : i10n.cardSwitch[userLang]}`
	);
...
export function remainingCards(
	...
	return remainingCards.length
		? getNextCard(plugin, remainingCards, cardStat, cardList)
		: undefined;
}
