...
export class CardList {
	...
	private parseLine(line: string): Card | null {
		...
		const { transcription, explanation } =
			this.parseTranscriptionAndExplanation(rest);
		...
		if (explanation !== '') {
			try {
				...
				console.warn(
					`Error parsing card: ${trimmedWord} : ${line}\n${error.message}`
				);
				new Notice(
					`Error parsing card: ${trimmedWord} : ${line}\n${error.message}`
				);
	...
	private parseTranscriptionAndExplanation(rest: string): {
